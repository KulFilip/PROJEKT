...
class ForexFeatures:
    ...
    def _volume_extremes(df):
        ...
        for w in [20, 60, 180]:
            ...
            df[f'vol_percentile_{w}'] = df['tick_volume'].rolling(w).rank(pct=True)
        
        for w in [20, 60]:
            vol_mean = df['tick_volume'].rolling(w).mean()
            vol_std = df['tick_volume'].rolling(w).std()
    ...
    def _impulse(df):
        ...
        for span in [5, 10, 15]:
            ...
            df[f'imp{span}_norm'] = df[f'imp{span}'] / (abs(df[f'imp{span}']).rolling(20).mean() + 1e-6)
    ...
    def _spread_analysis(df):
        ...
        for w in [10, 20, 60]:
            df[f'spread_avg_{w}'] = df['spread'].rolling(w).mean()
            df[f'spread_ratio_{w}'] = df['spread'] / (df[f'spread_avg_{w}'] + 1e-6)
            df[f'spread_percentile_{w}'] = df['spread'].rolling(w).rank(pct=True)
        
        spread_mean_20 = df['spread'].rolling(20).mean()
        spread_std_20 = df['spread'].rolling(20).std()
    ...
    def _effort_result_divergences(df):
        ...
        df['vsr_avg_20'] = df['vsr'].rolling(20).mean()
    ...
    def _wyckoff_patterns(df):
        ...
        df['up_test_count_20'] = df['up_test'].rolling(20).sum()
        df['down_test_count_20'] = df['down_test'].rolling(20).sum()
    ...
    def _climax_detection(df):
        ...
        df['climax_score'] = df['climax_score'].rolling(3).mean()
...
class Analyzer:
            ...
            for j in range(len(subset)):
                ...
                if j > 0:
                    ...
                    row.extend([
                        ...
                        s['intensity'] / (prev['intensity'] + 1e-6),
                    ])
                else:
    ...
    def detect_reversal_signatures(swings_df, df, tolerance=0.015):
        ...
        Identifies 'High Quality' reversal zones.
        """
        ...
        for _, row in extremes.iterrows():
            ...
            reversals.append({
                ...
                'vsa_signal': 'Climax' if is_climax else ('Anomaly' if is_anomaly else ('Test' if is_test else 'None')),
...
class DashboardBuilder:
    ...
    def plot_reversal_analytics(df, reversals, symbol='Unknown'):
        ...
        if not reversals.empty:
            ...
            if not tops.empty:
                fig.add_trace(go.Scatter(x=tops['time'], y=tops['price'] * 1.001,
            ...
            if not bots.empty:
                fig.add_trace(go.Scatter(x=bots['time'], y=bots['price'] * 0.999,
