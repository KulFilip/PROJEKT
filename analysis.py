...
class Analyzer:
    ...
    def prepare_ml_features(swings_df, window=10):
        ...
        Returns:
            X: Feature matrix for training (all but last if predicting future)
            y: Target matrix
            latest_X: Features for the NEXT swing (unseen target)
        """
        if len(swings_df) < window:
        ...
        for i in range(window, len(swings_df)):
            ...
            for j in range(len(subset)):
                ...
                if j > 0:
                    ...
                    row.extend([s['range'] / prev_s['range'] if prev_s['range'] != 0 else 1.0, 
                                s['duration_mins'] / prev_s['duration_mins'] if prev_s['duration_mins'] != 0 else 1.0])
                else:
                    row.extend([1.0, 1.0])
                row.append(s['range'] / s['duration_mins'] if s['duration_mins'] != 0 else 0)
            
            features.append(row)
            # Targets: range, duration_mins, and relative price delta %
        ...
        # Features for the FUTURE swing (based on the very latest 'window' swings)
        ...
        for j in range(len(latest_subset)):
            ...
            if j > 0:
                prev_s = latest_subset.iloc[j-1]
                latest_row.extend([s['range'] / prev_s['range'] if prev_s['range'] != 0 else 1.0, 
                                   s['duration_mins'] / prev_s['duration_mins'] if prev_s['duration_mins'] != 0 else 1.0])
            else:
                latest_row.extend([1.0, 1.0])
            latest_row.append(s['range'] / s['duration_mins'] if s['duration_mins'] != 0 else 0)
    ...
    def predict_next_swing_lstm(swings_df, window=10):
        """
        Predicts next swing using an LSTM model with scaling. Correctly handles future prediction.
        """
        ...
        # Scaling the latest feature vector for future prediction
        latest_X_scaled = scaler_X.transform(latest_X_raw)
        latest_X = latest_X_scaled.reshape(1, window, 8)
        
        X_lstm = X_scaled.reshape(X_scaled.shape[0], window, 8)
        
        model = Sequential([
            LSTM(64, input_shape=(window, 8), return_sequences=True),
            Dropout(0.2),
        ...
        model_p = Sequential([
            LSTM(64, input_shape=(window, 8), return_sequences=False),
    ...
    def calculate_exhaustion_score(df, swings_df, df_corr=None, config=None):
        ...
        # Binary Targets for Reversal Points
        df['is_turning_point'] = 0
        for _, swing in swings_df.iterrows():
            idx = df[df['time'] <= swing['end_time']].index[-1:]
            if not idx.empty:
                df.loc[idx, 'is_turning_point'] = 1
